...
de_clusters <- de_genes(seurat_input = organoid.combined.sct.neuron, r = 0.02, model_formula = formde, pseudo_factors =  c("stim", "replicate", "cell.line"))

length(unique(c(rownames(topTable(de_clusters[["0"]], coef="stimoxidation", number = Inf, p.value=0.05)), rownames(topTable(de_clusters[["1"]], coef="stimoxidation", number = Inf, p.value=0.05)), rownames(topTable(de_clusters[["2"]], coef="stimoxidation", number = Inf, p.value=0.05)))))

length(c(rownames(topTable(de_clusters[["0"]], coef="stimoxidation", number = Inf, p.value=0.05)), rownames(topTable(de_clusters[["1"]], coef="stimoxidation", number = Inf, p.value=0.05)), rownames(topTable(de_clusters[["2"]], coef="stimoxidation", number = Inf, p.value=0.05))))
```
...
##Markers for RNAScope
What's special about highly responsive clusters?  One possibility is that they are spatially arranged in an organoid such that they are more or less exposed to environmental changes.  In order to assess this, we need some histological data, but many of the markers for individual clusters are just bad.  We can use RNAScope, instead, to look at mRNA for individual genes that are reasonably highly expressed, and reasonably specific for individual clusters.  It makes sense to employ this for genes for which good antibodies are _not_ available.

To start, I'll look for cluster markers specifically in control (unstimulated) organoids, with somewhat more stringent thresholds.  Then I'll see whether good marker probes of a few clusters can be ordered as-is from RNAScope.

```{r}
organoid.combined.sct.neuron.unstim <- subset(organoid.combined.sct.neuron, subset= stim=="control")
markers4 <- FindMarkers(object=organoid.combined.sct.neuron.unstim, only.pos=TRUE, ident.1 = 4,  min.pct = 0.5)
markers4 %>% top_n(n=10, wt=avg_logFC)
```
This implies that the best marker is probably CACYBP or perhaps CREBRF.

I'll try again with the FindAllMarkers() function
```{r}
markers <- FindAllMarkers(object = organoid.combined.sct.neuron.unstim, only.pos = TRUE, logfc.threshold = 3, min.pct = 0.3)
markers %>% group_by(cluster) %>% top_n(n = 5, wt = avg_logFC) %>% kable()

FeaturePlot(organoid.combined.sct.neuron, features = c("rna_CACYBP", "rna_CREBRF"), max.cutoff = 10)
DimPlot(organoid.combined.sct.neuron)

```
I will want markers for cluster 4 (CACYBP), 6, 1, and 7.


```{r}
FeaturePlot(organoid.combined.sct.neuron, features = c("rna_TRIM17", "rna_SLC6A7", "rna_BTBD17", "rna_STMN4"))
# FeaturePlot(organoid.combined.sct.neuron, features = c("rna_EGR1", "rna_CASP3", "rna_CASP6", "rna_IGFBP1"))
# FeaturePlot(organoid.combined.sct.neuron, features = c("rna_TP73", "rna_JUN"), max.cutoff = 30)
...
FindAllMarkers(organoid.combined.sct.neuron, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
...
I'll now try again with more conservative clustering.
```{r}
organoid.combined.sct.neuron.unstim <- RunPCA(organoid.combined.sct.neuron.unstim, npcs = 100)
organoid.combined.sct.neuron.unstim <- RunUMAP(organoid.combined.sct.neuron.unstim, reduction = "pca", dims = 1:50)
organoid.combined.sct.neuron.unstim <- FindNeighbors(organoid.combined.sct.neuron.unstim, dims=1:100)
organoid.combined.sct.neuron.unstim <- FindClusters(organoid.combined.sct.neuron.unstim, resolution=0.05)
